@props(['name', 'label' => 'Tags', 'initialTags' => [], 'availableTags' => [], 'placeholder' => 'Add tags...'])

<div x-data="{
    allTags: {{ json_encode($availableTags) }},
    selectedTags: {{ json_encode($initialTags) }},
    searchTerm: '',
    showSuggestions: false,
    activeSuggestionIndex: -1,
    get filteredTags() {
        filtered = this.allTags.filter(tag => {
            return tag.toLowerCase().includes(this.searchTerm.toLowerCase()) && !this.selectedTags.includes(tag);
        });
    ...
    <div class="mt-2 relative">
        ...
        <input type="text" x-model="searchTerm" x-ref="tagInput" @focus="handleInputFocus()"
            ...
            class="block w-full rounded-md px-2 py-1.5 text-gray-900 ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 focus-within:outline-none sm:text-sm sm:leading-6"
        ...
        <div x-show="showSuggestions && filteredTags.length > 0" x-transition:enter="transition ease-out duration-100"
            ...
            class="absolute z-10 mt-1 w-full max-h-60 overflow-auto rounded-md bg-white py-1 shadow-lg ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 ring-opacity-5 focus:outline-none sm:text-sm">
