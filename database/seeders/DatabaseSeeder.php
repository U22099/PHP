...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Ensure each freelancer bids for a different job
        $freelancers = User::where('role', 'freelancer')->get();
        $jobs = Job::all();

        foreach ($jobs as $job) {
            $bidded = [];
            while (count($bidded) < 10) {
                if ($freelancers->count() === count($bidded)) {
                    break;
                }
                $freelancer = $freelancers->filter(function ($f) use ($bidded) {
                    return !in_array($f->id, $bidded);
                })->random();

                $bidded[] = $freelancer->id;
                Bids::factory()->create([
